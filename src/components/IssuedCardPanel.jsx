...
export default function IssuedCardPanel({ app, onUpdate }) {
  ...
  const handleCopy = () => {
    if (app.virtualCardNumber) {
      navigator.clipboard.writeText(app.virtualCardNumber).then(() => {
        setCopied(true)
    ...
    <div className="card p-6">
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        <div className="bg-citizens-green-light rounded-lg p-3">
          ...
          {app.activationStatus === 'ACTIVE' && app.activatedAt && (
            <div className="text-xs text-citizens-gray mt-0.5">{fmtDate(app.activatedAt)}</div>
          )}
        </div>
        <div className="bg-citizens-green-light rounded-lg p-3">
      ...
      {/* Activate button */}
      {app.activationStatus === 'PENDING' && (
        <button
          onClick={handleActivate}
          disabled={activating}
          className="mt-3 w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm rounded-lg transition-colors disabled:opacity-50"
        >
          {activating ? (
      ...
      {app.activationStatus === 'ACTIVE' && (
        <div className="mt-3 flex items-center justify-center gap-2 py-2.5 bg-green-50 border border-green-200 rounded-lg">
          <span className="text-green-600 font-semibold text-sm">Card Active ✓</span>
        </div>
      )}

      {/* Virtual Card */}
      {app.virtualCardNumber && (
        <div className="mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-1">
            <div className="text-xs font-semibold text-blue-700">Virtual Card</div>
            <span className="text-xs text-blue-500">Ready for online use immediately</span>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span className="font-mono text-sm text-blue-800 tracking-widest flex-1">{app.virtualCardNumber}</span>
            <button
              onClick={handleCopy}
              className="text-xs text-blue-600 hover:text-blue-800 border border-blue-300 rounded px-2 py-1 transition-colors"
            >
              {copied ? 'Copied!' : 'Copy'}
            </button>
          </div>
      ...
      <div className="mt-4 p-3 bg-citizens-green-pale border border-citizens-green/20 rounded-lg flex gap-2.5">
        ...
        <p className="text-xs text-citizens-green-dark leading-relaxed">
          Your card is on its way. You'll receive your physical card within 5–7 business days. Use your virtual card for online purchases immediately.
        </p>
      </div>
    </div>
  )
